...
# %% auto 0
__all__ = ['logger', 'int_to_excel_col', 'LabelToVar', 'word_overlap', 'regex', 'Condition', 'TestNode', 'TestCase',
...
def get_possible_assignments(dist,test_case:TestCase,label_to_var:LabelToVar):
    ...
    for col_idx,node in enumerate(test_case.test_nodes):
        ...
        if not viable_trace_row_nums:
            logger.warning(f"No viable trace row nums for expected trace {node.label}")
            return None
...
# %% ../nbs/016_event_stream_warping.ipynb 58
